...
![Image explaining CIDR matching](cidr.png 'see how the prefixes match?')
...
this, one of the CIDR prefixes that Cloudflare
[announces](https://bgp.tools/prefix/1.1.1.0/24) is `1.1.1.0/24` for their
famous `1.1.1.1` DNS service.
...
prefix searches. The reason why this works is because it iterates over the IP
bit by bit and chooses a side of the tree (zero or one) and follows it to the
next node. In practice, this is rather fast.

Let's start off by writing a node struct for each vertex in our trie (tree)
graph.

```zig title="TrieNode.zig" attribution="Node struct"
...
```zig title="TrieParent.zig" attribution="Trie Parent and Boilerplate"
...
All that's left now is to write some insertion and querying functions. This is
all inside the `TrieParent` struct.

```zig title="TrieParent.zig" attribution="Insert and Query (inside TrieParent)"
...
pub fn insert(self: *@This(), ip: u32, length: u5, asn: i32) !void {
    ...
    // one and set the array's singular value to the asn and
...
Ok that wasn't the best implementation. I'll follow this post up and link it
here when it's complete for all the optimizations I came up for this.
